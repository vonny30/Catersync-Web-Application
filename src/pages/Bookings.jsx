...
export default function Bookings() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (bookingsData && bookingsData.length > 0) {
        ...
        if (bookingIds.length > 0) {
          ...
          for (const p of paymentsData) {
            ...
            // Only entries that move the books count (v_payment_ledger's
            // counts_in_ledger): not unverified claims, not reversals, not
            // reversed receipts. A reversal is never a refund.
            if (amount > 0 && movesBooks(p)) {
              paymentsMap[p.booking_id].positive += amount;
              if (p.pay_status === RECEIPT_STAGES.deposit) {
      ...
      // Paged: first_name is not unique, so customer_id carries the ordering
      // the rest of the way. Without it, paging repeats and skips rows.
      const customersList = await fetchAllRows(() => supabase
        .from('customer')
...
const handleMarkCompleted = async (id) => {
  ...
  try {
    ...
    // 4. // Receipts are NOT relabelled on completion. This used to turn every
  ...
  return (
    <div className="space-y-[18px] relative">
          ...
          <table className="w-full table-fixed text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
              {loading ? (
                ...
                <tr><td colSpan="11"><EmptyResult canClear={canClearFilters} onClear={clearFilters} /></td></tr>
              ) : (
                bookings.map((booking) => {
      ...
      {isApprovalModalOpen && approvalBooking && (
        <ApprovalModal
          key={approvalBooking.booking_id}
