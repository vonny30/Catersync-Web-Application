...
{order.booking_status === 'Completed' && positivePayments < (order.total_amount || 0) && (
  <span className="px-4 py-1.5 rounded-full text-xs font-bold border bg-amber-50 border-amber-200 text-amber-700">
    ⚠️ Balance Remaining
          ...
          {order.notes && (
            <div className="flex items-start gap-2.5 mt-[22px] px-4 py-3.5 bg-[#fbfcfd] border border-[#eef2f6] rounded-xl">
              ...
              <div className="min-w-0">
                ...
                {/* Machine-written "[REFUND] Amount: ..." marker stripped from
                    DISPLAY only — the stored value is shared with the customer
                    mobile app and is left alone. */}
                <p className="mt-1 text-[13.5px] leading-[1.5] text-slate-700 whitespace-pre-wrap [text-wrap:pretty]">{displayNotes(order.notes)}</p>
              </div>
            </div>
          )}
        </SectionCard>
      ...
      {/* ================= LISTS ================= */}
        {/* Same row structure as Booking Details: one 12-column grid per row,
            every row on the same 7/5 spans, so the seam between the columns
            sits at the same x all the way down. Explicit rows rather than one
        ...
        />
              ...
              {positivePayments > 0 && !isRefundable && downpaymentPaid > 0 && (
                <div className="border-t border-slate-200 pt-3 mt-3 text-xs text-slate-500">
                  <p>⚠️ This order is <strong>non‑refundable</strong> because the event is less than 3 days away. The deposit of ₱{downpaymentPaid.toLocaleString()} will be forfeited.</p>
                </div>
              )}
