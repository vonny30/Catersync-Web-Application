...
export function useCancellationHandlers({ booking, payments, fetchData }) {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      // unreturned row on a cancelled booking is then counted as equipment that
      // is out. That is exactly how BKG-105 came to hold 59 phantom units.
      const { error: equipReleaseError } = await supabase.from('booking_equipment').delete().eq('booking_id', booking.booking_id).eq('returned', false);
      ...
      const { error: vehicleReleaseError } = await supabase.from('vehicle_assign').delete().eq('booking_id', booking.booking_id).neq('assignment_status', 'Completed');
