...
- **`vehicle_assign_no_exact_duplicate`** — a unique index on `(booking_id, vehicle_id, dispatch_datetime)`, added 30 Aug 2026. Keyed on the dispatch time on purpose: a booking still holds a setup row **and** a collection row for the same vehicle, because the legs leave at different times. It rejects only an exact repeat — a double-submit or a retried request. All three insert paths treat `23505` as "already there", not as a failure; the two manual ones say so in plain language rather than surfacing a constraint name.
