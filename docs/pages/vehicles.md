...
- Short orders are **pickup or delivery**, derived from `delivery_fee` (> 0 means delivery) because no field records it. A customer pickup needs no vehicle, so it is skipped by the "Needs a vehicle" queue and auto-allocates nothing — the approval panel still lists every van so a manager can add one when a delivery fee was simply waived.
