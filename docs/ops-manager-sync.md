...
| 20 Sep 2026 | **`confirmed_at` and `completed_at` are not reliably written.** Nothing sets `completed_at` at all, and a manual confirm from the web app writes `booking_status` alone | Do not display or sort on either column. Use `booking_status_log` for when a status changed |
