...
|---|---|---|
| 20 Sep 2026 | **`booking_status_log` is now read by the web app** (Status History on both detail pages) and written by trigger on every status change, whatever caused it: `Manual`, `Auto-confirm (50% threshold)`, `Auto-revert (reversal below 50% threshold)`, `Manager override`. A status override goes through `f_override_booking_status`, never a direct `booking_status` write | Read it rather than guessing why a booking moved. **Note the ordering trap**: `changed_at` is `now()`, the transaction clock, so every row written inside one transaction carries the identical timestamp and `log_id` is a random uuid, useless as a tiebreak. **Sort by `seq`** (identity column, added 20 Sep, indexed `(booking_id, seq desc)`), never `changed_at`: `changed_at` is `now()`, so every row written inside one transaction carries the identical instant and `log_id` is a random uuid. The column's own comment repeats this |
| 20 Sep 2026 | **A reversal can demote a booking.** `f_maybe_unconfirm_booking` reverts Confirmed -> Approved when verified receipts fall below 50%, sets `flagged_for_review` with a reason, clears `confirmed_at`, and **keeps equipment and vehicle assignments in place** | **Breaking for any OM screen that treats Confirmed as one-way.** A booking on today's list can be Approved tomorrow with its assignments still attached; the flag is the signal that a manager has not looked yet |
| 20 Sep 2026 | **Status timestamps are now stamped database-side** by `trg_stamp_status_times`: `approved_at`, `confirmed_at`, `completed_at`, `closed_at` on entry to each status, never overwriting an existing value. Before this nothing wrote `completed_at` at all and a manual confirm left `confirmed_at` null. `confirmed_at` was backfilled from the receipt that first crossed 50%; `completed_at` was left null on the four legacy rows, because nothing records when they finished and a guessed timestamp in a factual column cannot be told apart from a real one afterwards | Safe to display from now on, but **an older row's `completed_at` can still be null** — render that as "not recorded", never as a blank or an invented date. A demotion clears `confirmed_at`, so a re-confirmation re-stamps it |
| 18 Sep 2026 | **Vehicle runs can be edited and removed** from both detail pages: a Scheduled run's vehicle or departure time can change (same leg, same bounds and conflict checks), and a Scheduled run can be removed. Completed runs are never touched. Short-order dispatch now follows `isResourceLocked` too | **Breaking for any OM itinerary cached per run.** A `vehicle_assign` row's `vehicle_id` and `dispatch_datetime` can change, and a row can disappear. There is no vehicle change log; re-read the table (it is in the realtime publication) |
| 18 Sep 2026 | **Equipment and dispatch stay editable on Confirmed bookings.** The web app's lock moved from `isPaymentLedgerLocked` to `isResourceLocked` (`utils/resourceLock.js`): locked on Pending, Completed, Cancelled and Rejected only. Quantities can now go **down** as well as up, never below `returned_quantity`. Every change is recorded by trigger in `booking_equipment_log` | **Breaking for any OM screen that treats a Confirmed booking's allocation as final.** A loading list read on Thursday can change on Friday. Re-read `booking_equipment` (it is in the realtime publication) rather than caching it at confirmation |
